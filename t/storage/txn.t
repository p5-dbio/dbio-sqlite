...
{
  my $schema = DBIO::SQLite::Test->init_schema(sqlite_use_file => 1, no_deploy => 1);
...

{
  ...
  # Test failed txn_do() with failed rollback
  {
    ...
    isa_ok($cd, 'DBIO::Test::CD', q{failed txn_do with a failed txn_rollback }.
...
warnings_are {
  my $factory = DBIO::SQLite::Test->init_schema;
  ...
  my $schema = DBIO::Test::Schema->connect (sub { $dbh });
...
{
  package DBIO::Test::BrokenOverload;
...

{
  ...
  local $SIG{__WARN__} = sub {
    $_[0] =~ /\QExternal exception class DBIO::Test::BrokenOverload implements partial (broken) overloading preventing its instances from being used in simple (\E\$x eq \$y\Q) comparisons/
      ? push @w, @_
      : warn @_
  };
  ...
  my $broken_exception = bless {}, 'DBIO::Test::BrokenOverload';
