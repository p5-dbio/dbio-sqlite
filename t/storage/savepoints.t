...
for ('', keys %$env2optdep) { SKIP: {
  ...
  if ($prefix = $_) {
    ...
    $schema = DBIO::Test::Schema->connect ($dsn,$user,$pass,{ auto_savepoint => 1 });
    ...
    $schema = DBIO::SQLite::Test->init_schema(no_populate => 1, auto_savepoint => 1 );
  }

  note "Testing $prefix";

  local $schema->storage->{debugobj} = my $stats = DBIO::Test::SVPTracerObj->new;
