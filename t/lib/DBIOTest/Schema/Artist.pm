...
__PACKAGE__->has_many(
    cds => 'DBIOTest::Schema::CD', undef,
    { order_by => { -asc => 'year'} },
);

__PACKAGE__->has_many(
  cds_cref_cond => 'DBIOTest::Schema::CD',
  sub {
...
__PACKAGE__->has_many(
    cds_unordered => 'DBIOTest::Schema::CD'
);
__PACKAGE__->has_many(
    cds_very_very_very_long_relationship_name => 'DBIOTest::Schema::CD'
);

__PACKAGE__->has_many( twokeys => 'DBIOTest::Schema::TwoKeys' );
__PACKAGE__->has_many( onekeys => 'DBIOTest::Schema::OneKey' );

__PACKAGE__->has_many(
  artist_undirected_maps => 'DBIOTest::Schema::ArtistUndirectedMap',
  [ {'foreign.id1' => 'self.artistid'}, {'foreign.id2' => 'self.artistid'} ],
  { cascade_copy => 0 } # this would *so* not make sense
);

__PACKAGE__->has_many(
    artwork_to_artist => 'DBIOTest::Schema::Artwork_to_Artist' => 'artist_id'
);
__PACKAGE__->many_to_many('artworks', 'artwork_to_artist', 'artwork');

__PACKAGE__->has_many(
    cds_without_genre => 'DBIOTest::Schema::CD',
    sub {
        # This is for test purposes only. A regular user does not
        # need to sanity check the passed-in arguments, this is what
        # the tests are for :)
