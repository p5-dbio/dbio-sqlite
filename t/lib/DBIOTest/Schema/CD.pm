...
use base 'DBIOTest::BaseResult';
use DBIOTest::Util 'check_customcond_args';
...
__PACKAGE__->belongs_to( artist => 'DBIOTest::Schema::Artist', undef, {
    is_deferrable => 1,
    proxy => { artist_name => 'name' },
});
__PACKAGE__->belongs_to( very_long_artist_relationship => 'DBIOTest::Schema::Artist', 'artist', {
    is_deferrable => 1,
});

# in case this is a single-cd it promotes a track from another cd
__PACKAGE__->belongs_to( single_track => 'DBIOTest::Schema::Track',
  { 'foreign.trackid' => 'self.single_track' },
  { join_type => 'left'},
);

__PACKAGE__->belongs_to( single_track_opaque => 'DBIOTest::Schema::Track',
  sub {
...
__PACKAGE__->belongs_to( existing_single_track => 'DBIOTest::Schema::Track',
  { 'foreign.trackid' => 'self.single_track' },
);

__PACKAGE__->has_many( tracks => 'DBIOTest::Schema::Track' );
__PACKAGE__->has_many(
    tags => 'DBIOTest::Schema::Tag', undef,
    { order_by => 'tag' },
);
__PACKAGE__->has_many(
    cd_to_producer => 'DBIOTest::Schema::CD_to_Producer' => 'cd'
);

__PACKAGE__->has_many( twokeys => 'DBIOTest::Schema::TwoKeys', 'cd' );


# the undef condition in this rel is *deliberate*
# tests oddball legacy syntax
__PACKAGE__->might_have(
    liner_notes => 'DBIOTest::Schema::LinerNotes', undef,
    { proxy => [ qw/notes/ ] },
);
__PACKAGE__->might_have(artwork => 'DBIOTest::Schema::Artwork', 'cd_id');
__PACKAGE__->has_one(mandatory_artwork => 'DBIOTest::Schema::Artwork', 'cd_id');
...
__PACKAGE__->belongs_to('genre', 'DBIOTest::Schema::Genre',
    'genreid',
    {
        join_type => 'left',
        on_delete => 'SET NULL',
...
__PACKAGE__->belongs_to('genre_inefficient', 'DBIOTest::Schema::Genre',
