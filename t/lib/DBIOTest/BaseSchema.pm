...
use strict;
use warnings;
use base qw(DBIOTest::Base DBIO::Schema);
...
sub capture_executed_sql_bind {
  ...
  local $self->storage->{debugobj} = my $tracer_obj = DBIOTest::SQLTracerObj->new;
...
END {
  ...
  if ($locker->{lock_name} and ($ENV{DBIOTEST_LOCK_HOLDER}||0) == $$) {
...
sub connection {
  ...
  if( $ENV{DBIOTEST_SWAPOUT_SQLAC_WITH} ) {

    my( $sqlac_like ) = $ENV{DBIOTEST_SWAPOUT_SQLAC_WITH} =~ /(.+)/;
    ...
    push @{ $args[-1]{on_connect_call} }, (
      [ rebase_sqlmaker => 'DBIOTest::SQLAC::SwapOut' ],
    );
  }
...
# DBIOTEST_MSSQL_DSN=dbi:Sybase:server=192.168.0.11:1433;database=dbtst
#  and
# DBIOTEST_MSSQL_ODBC_DSN=dbi:ODBC:server=192.168.0.11;port=1433;database=dbtst;driver=FreeTDS;tds_version=8.0
# are the same server
...
# just pretend we are python you know? :)
  ...
  if (
    ! $DBIOTest::global_exclusive_lock
      and
    ( ! $ENV{DBIOTEST_LOCK_HOLDER} or $ENV{DBIOTEST_LOCK_HOLDER} == $$ )
      and
    ref($args[0]) ne 'CODE'
      and
    ($args[0]||'') !~ /^ (?i:dbi) \: SQLite (?: \: | \W ) .*? (?: dbname\= )? (?: \:memory\: | t [\/\\] var [\/\\] DBIO\-) /x
  ) {
...

    {
      ...
      local $ENV{DBIOTEST_LOCK_HOLDER} = -1;
    ...
    if ($locktype and (!$locker or $locker->{type} ne $locktype)) {
      ...
      my $lockpath = DBIOTest::RunMode->tmpdir->file("_dbictest_$locktype.lock");
