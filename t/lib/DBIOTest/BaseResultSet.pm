package #hide from pause
  DBIOTest::BaseResultSet;

use strict;
use warnings;

BEGIN {
  my @subclassing = qw(DBIOTest::Base DBIO::ResultSet);

  if( ! $ENV{DBIOTEST_MOOIFIED_RESULTSETS} ) {
...
#
# Ensure that unless we are called from a test - DBIO always fills it in
sub BUILDARGS {
  if(
    ! defined $_[2]
      and
