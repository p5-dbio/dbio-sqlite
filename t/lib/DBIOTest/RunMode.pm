...
BEGIN {
  ...
  delete $ENV{DBIODEVREL_SWAPOUT_SQLAC_WITH};
}

use Fcntl ':DEFAULT';
use File::Spec ();
use File::Temp ();
use DBIOTest::Util 'local_umask';
use DBIO::Util qw(dir_path file_path parent_dir mkpath);

_check_author_makefile() unless $ENV{DBIOTEST_NO_MAKEFILE_VERIFICATION};
...
sub tmpdir {
  $tmpdir ||= do {
    ...
    if (@parts == 2 and $parts[1] =~ /^ [ \\ \/ ]? $/x ) {
      ...
      local $@;
      my $u = local_umask(0); # match the umask we use in DBIOTest(::Schema)
...
sub _check_author_makefile {
  ...
  # DBIO: Module::Install checks disabled — migrating to Dist::Zilla
  return;
...
By default DBIO's Makefile.PL turns all optional dependencies into
