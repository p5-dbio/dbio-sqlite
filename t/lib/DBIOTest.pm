...
BEGIN {
  ...
  my $this_file = Cwd::abs_path(__FILE__) || __FILE__;
  my $tlib_dir  = File::Basename::dirname($this_file);
  my $repo_root = Cwd::abs_path(
    File::Spec->catdir($tlib_dir, File::Spec->updir, File::Spec->updir)
  ) || '.';

  # Allow local multi-repo development without requiring an installed DBIO.
  my @dbio_roots = grep { defined($_) && length($_) } (
    $ENV{DBIO_REPO_DIR},
    Cwd::abs_path(File::Spec->catdir($repo_root, File::Spec->updir, 'dbio')),
  );

  for my $root (@dbio_roots) {
    next unless -d $root;
    for my $inc (
      File::Spec->catdir($root, 'lib'),
      File::Spec->catdir($root, 't', 'lib'),
    ) {
      my $abs = Cwd::abs_path($inc) || next;
      next unless -d $abs;
      unshift @INC, $abs unless grep { $_ eq $abs } @INC;
    }
  }
}

use DBIOTest::Util qw( local_umask await_flock dbg DEBUG_TEST_CONCURRENCY_LOCKS );
use DBIOTest::Schema;
use DBIOTest::Util::LeakTracer qw/populate_weakregistry assert_empty_weakregistry/;
...
DBIOTest - Library to be used by DBIO test scripts
...
using C<DBIOTest> grabs a shared lock, and the few tests that request a
...
This method removes the test SQLite database in t/var/DBIO.db
...
sub import {
    ...
    my $lockpath = file_path(DBIOTest::RunMode->tmpdir, '_dbictest_global.lock');
...
sub _cleanup_dbfile {
    ...
    if (
      ...
      $ENV{DBIOTEST_LOCK_HOLDER} == $$
        ...
        if ($need_global_cleanup and my $dbh = DBIOTest->schema->storage->_dbh) {
          $dbh->disconnect;
        }
...
sub init_schema {
    ...
    if ($args{compose_connection}) {
      ...
      $schema = DBIOTest::Schema->compose_connection(
                  'DBIOTest', $self->_database(%args)
                );
    } else {
      $schema = DBIOTest::Schema->compose_namespace('DBIOTest');
    }
...
=head2 deploy_schema
  ...
  DBIOTest->deploy_schema( $schema );

This method does one of two things to the schema.  It can either call
the experimental $schema->deploy() if the DBIOTEST_SQLT_DEPLOY environment
...
=head2 populate_schema
  ...
  DBIOTest->populate_schema( $schema );
