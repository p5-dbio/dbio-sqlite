...
{
  package DBIO::Test::Schema::BigIntArtist;
  use base 'DBIO::Test::Schema::Artist';
  __PACKAGE__->table('artist');
  __PACKAGE__->add_column(bigint => { data_type => 'bigint' });
}
$schema->register_class(BigIntArtist => 'DBIO::Test::Schema::BigIntArtist');
...
for my $bi ( qw(
  ...
  local $SIG{__WARN__} = sub {
    ...
    elsif ($_[0] =~ /An integer value occupying more than 32 bits was supplied .+ can not bind properly so DBIO will treat it as a string instead/ ) {
